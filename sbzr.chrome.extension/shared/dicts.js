(function (global) {
  ...
  const DICTS = {
    ...
    EDITOR_RIME_PATHS: TABLES.filter((table) => !table.prefix && table.editorEnabled !== false).map((table) => table.path),
    EDITOR_AFFIX_SOURCES: TABLES
      .filter((table) => table.prefix && table.editorEnabled !== false)
      .map((table) => ({ path: table.path, prefix: table.prefix })),
